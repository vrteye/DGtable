"""
其他应收账款-按账龄披露 other_receivable_account_age_reveal
"""
import requests
import numpy as np
from basic import *

def Otherreceivableaccountagereveal(path, tradeKey):
    # print(tradeKey)
    try:
        f = open(path, "r", encoding="utf-8").read()
        f = get_str_btw(f, '、合并财务报表项目注释', '、合并范围的变更')
        f = get_str_btw(f, '、其他应收款', '、现金流量表项目')
        ...
        # print(data)
        Success = requests.post('http://192.168.1.200:9008/otherReceivableAccountAgeReveal/add', json=data)
